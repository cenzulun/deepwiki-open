...
LINGYI_API_KEY = os.environ.get('LINGYI_API_KEY')
...
def load_generator_config():
    ...
    # 动态加载国产模型配置（如果启用）
    enable_chinese_models = os.environ.get('ENABLE_CHINESE_MODELS', 'false').lower() == 'true'
    if enable_chinese_models:
        chinese_models_config = load_json_config("chinese_models.json")
        if chinese_models_config and "providers" in chinese_models_config:
            if "providers" not in generator_config:
                generator_config["providers"] = {}
            generator_config["providers"].update(chinese_models_config["providers"])
            logger.info("已加载国产模型配置")
    ...
    if "providers" in generator_config:
        for provider_id, provider_config in generator_config["providers"].items():
            ...
            # 根据provider_id使用默认映射
            elif provider_id in ["google", "openai", "openrouter", "ollama", "bedrock", "azure", "dashscope", "zhipuai", "deepseek"]:
                default_map = {
            ...
            # 特殊处理国产模型
            elif provider_id in ["moonshot", "wenxin", "lingyi", "minimax", "doubao", "stepfun", "xunfei"]:
                provider_config["model_client"] = ChineseModelsClient
                # 设置提供商名称用于创建客户端实例
                provider_config["chinese_provider"] = provider_id
            else:
...
def get_model_config(provider="google", model=None):
    ...
    Parameters:
        provider (str): 模型提供商 ('google', 'openai', 'openrouter', 'ollama', 'bedrock', 'zhipuai', 'deepseek', 'moonshot', 'wenxin', etc.)
        model (str): 模型名称，如果为None则使用默认模型

    Returns:
        dict: 包含model_client、model和其他参数的配置
    """
    # 获取提供商配置
    ...
    if provider == "ollama":
    ...
    elif provider in ["moonshot", "wenxin", "lingyi", "minimax", "doubao", "stepfun", "xunfei"]:
        # 国产模型需要特殊的客户端初始化参数
        chinese_provider = provider_config.get("chinese_provider", provider)
        result["model_kwargs"] = {
        ...
        # 创建客户端实例而不是类
        result["model_client"] = model_client(chinese_provider)
    else:
        # 其他提供商的标准结构
