...
export default function Home() {
  ...
  const parseRepositoryInput = (input: string): {
    ...
    // 处理 Windows 绝对路径 (例如: C:\path\to\folder)
    const windowsPathRegex = /^[a-zA-Z]:\\(?:[^\\/:*?"<>|\r\n]+\\)*[^\\/:*?"<>|\r\n]*$/;
    // 处理 Unix/Linux 绝对路径 (例如: /path/to/folder)
    const unixPathRegex = /^\/(?:[^\/\0]+\/)*[^\/\0]*$/;
    // 处理相对路径 (例如: ./folder 或 ../folder)
    const relativePathRegex = /^(\.\.?\/|[^\\\/:*?"<>|\r\n\s][^\\\/:*?"<>|\r\n\s]*[\\\/])?[^\\\/:*?"<>|\r\n\s]*$/;
    // 扩展的 Git 仓库正则表达式，支持更多格式
    const customGitRegex = /^(?:https?:\/\/)?(?:www\.)?([^\/]+)\/(.+?)\/([^\/]+)(?:\.git)?\/?$/;
    // 简单的 owner/repo 格式（但要排除包含点号的相对路径）
    const ownerRepoRegex = /^([\w\-]+)\/([\w\-]+)$/;
    // 支持您的 GitHub 仓库格式
    const yourGitHubRegex = /^mac\/github-local\/([^\/]+)$/;
    ...
    // 处理 Unix/Linux 绝对路径
    else if (unixPathRegex.test(input)) {
      type = 'local';
      localPath = input;
    ...
    // 处理相对路径
    else if (relativePathRegex.test(input) && (input.includes('./') || input.includes('../') || !input.includes('://'))) {
      // 检查是否是相对路径但不是 URL
      if (!input.match(/^https?:\/\//) && !input.includes('.') && input.length > 0) {
        type = 'local';
        // 将相对路径转换为绝对路径
        try {
          if (typeof window !== 'undefined') {
            // 在浏览器环境中无法直接解析本地文件系统路径，保持原样
            localPath = input;
          } else {
            // 在 Node.js 环境中可以转换为绝对路径
            localPath = require('path').resolve(input);
          }
          repo = input.split(/[\/\\]/).pop() || 'local-repo';
          owner = 'local';
        } catch (error) {
        ...
        if (match) {
          ...
          console.error('不支持的输入格式:', input);
          return null;
        }
      }
    }
    // 特殊处理您的 GitHub 仓库格式
    else if (yourGitHubRegex.test(input)) {
      type = 'github';
      owner = 'mac';
      repo = input.match(yourGitHubRegex)![1];
    }
    // 标准的 owner/repo 格式
    else if (ownerRepoRegex.test(input) && !input.includes('://') && !input.includes('.')) {
      const match = input.match(ownerRepoRegex);
      if (match) {
        type = 'github';
        owner = match[1];
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (!parsedRepo) {
      setError('无效的仓库格式。请使用 "owner/repo" 格式、GitHub/GitLab/BitBucket URL，或本地文件夹路径如 "/path/to/folder" 或 "C:\\path\\to\\folder"。');
      return;
    }
